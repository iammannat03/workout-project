...
function EquipmentCard({ equipment, isSelected, onToggle, onViewExercises, showViewButton }: EquipmentCardProps) {
  const t = useI18n();
  const locale = useCurrentLocale();

  const translation = getEquipmentTranslation(equipment.value, t);
  const categorySlug = equipment.value.toLowerCase().replace("_", "-");

  const handleCardClick = (e: React.MouseEvent) => {
    // If clicking on the view button, don't toggle selection
    if ((e.target as HTMLElement).closest('.view-exercises-button')) {
      return;
    }
    onToggle();
  };
  ...
  return (
    ...
    >
      <CardContent className="p-2 sm:p-4 h-auto flex flex-col justify-center items-center relative">
        ...
        {showViewButton && (
          <Link
            href={`/${locale}/workouts/${categorySlug}`}
            className="view-exercises-button mt-2 w-full"
            onClick={(e) => e.stopPropagation()}
          >
            <div
              className={cn(
                "flex items-center justify-center gap-1 text-xs font-medium px-3 py-1.5 rounded-lg transition-all duration-200",
