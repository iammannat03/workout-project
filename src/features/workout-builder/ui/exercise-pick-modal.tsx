...
export function ExercisePickModal({ exercise, muscle, isOpen, onClose, onConfirmPick }: ExercisePickModalProps) {
  ...
  // Extraire les attributs utiles - Use string literals to avoid client-side enum bundling issues
  const equipmentAttributes = getExerciseAttributesValueOf(exercise, "EQUIPMENT" as ExerciseAttributeNameEnum);
  const typeAttributes = getExerciseAttributesValueOf(exercise, "TYPE" as ExerciseAttributeNameEnum);
  const mechanicsTypeValue = getExerciseAttributesValueOf(exercise, "MECHANICS_TYPE" as ExerciseAttributeNameEnum);
