...
export const useWorkoutBuilderStore = create<WorkoutBuilderState>((set, get) => ({
  ...
  nextStep: () => set((state) => ({ currentStep: Math.min(state.currentStep + 1, 2) as WorkoutBuilderStep })),
  ...
  fetchExercises: async () => {
    ...
    try {
      ...
      // If only equipment is selected (no muscles), use getExercisesByMuscleAction
      // This returns exercises grouped by all available muscles for that equipment
      ...
      // If both equipment and muscles are selected, use the regular action
      if (selectedEquipment.length > 0 && selectedMuscles.length > 0) {
